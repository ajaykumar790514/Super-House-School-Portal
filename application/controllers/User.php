...

    {
        ...
        switch ($action) {
            ...
            case null:
                ...
                $data['classs'] = $query = $this->db->where(['active'=>'1','is_deleted'=>'NOT_DELETED'])->get('class_master')->result();
               ...
               case 'profile':
                ...
                $this->data['user']=$customer = $this->user_model->get_row_data('customers','id',$user_id);

                $this->data['school'] = $query = $this->db->where(['group_id'=>$group_id,'active'=>'1','is_deleted'=>'NOT_DELETED'])->get('school_master')->result();
                $this->data['groups'] = $query = $this->db->where(['id'=>$customer->group_id,'active'=>'1','is_deleted'=>'NOT_DELETED'])->get('group_master')->result();
