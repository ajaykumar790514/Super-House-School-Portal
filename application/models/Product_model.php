...

{
...

    {
        ...
        ->select('t1.*,t1.name_portal as prod_name,t4.img,t4.id as cover_id,t3.mrp,t3.selling_rate,t5.name as cat_name,t3.qty as product_qty,t6.offer_upto,t6.discount_type,t7.is_featured')
   ...
   public function get_value($id)
...

    {
        ...
        ->select('t1.*,t1.name_portal as prod_name,t2.img,t2.thumbnail,t2.is_cover,t2.id as cover_id, IFNULL(t3.mrp, 0) as mrp, t3.id as inventory_id, t3.selling_rate, t3.qty, t4.product_id as wishlist_pid, t5.id as cart_id, IFNULL(t5.qty, 0) as cart_qty, t6.discount_type, t6.offer_upto,t6.offer_associated,t1.flag as ProductFlag,t1.id as proid')
       ...
       // ->join('flavour_master t7', 't7.id = t1.flavour_id AND t7.is_deleted="NOT_DELETED" AND t7.active="1"','left')
        ...
        ->where(['t3.id' => $pid,'t1.is_deleted'=>'NOT_DELETED','t1.active'=>'1'])
...

    {
        ...
        ->select('t1.selling_rate, t1.mrp, t2.tax_value as product_tax, t1.qty as stock_qty,t2.id as pid, t2.name_portal as product_name, t2.rating, t3.img, t4.offer_upto, t4.discount_type, t5.id as cart_id, t2.unit_type, t2.unit_value, t2.product_code,t3.thumbnail,t2.flag as ProductFlag')
        ...
        ->where(['t1.is_deleted' => 'NOT_DELETED','t2.is_deleted'=>'NOT_DELETED','t2.active'=>'1','t1.id' =>$pid])
...
//    function fetch_data($group_id,$class_id,$category_pro_id,$pro_id,$id,$cids,$shop_id,$brand,$prop_filter,$sort_by,$limit,$start,$is_count,$isAll)
...
//         ->select('t1.*, t1.name_portal as prod_name, t4.img,t4.thumbnail, t4.id as cover_id, t3.id as inventory_id, t3.mrp, t3.selling_rate, t3.qty as product_qty, t6.offer_upto, t6.discount_type,t1.flag as ProductFlag,t1.id as proid')
...
//         ->join('shops_coupons_offers t6', 't1.id=t6.product_id','left')
//         ->join('class_pro_maps as t11','t1.id=t11.pro_id','left')
//          ->join('cat_pro_maps t12', 't12.pro_id=t1.id','left')
//         ->join('products_category t13', 't13.id=t12.cat_id','left');
//         //->join('product_flags t7', 't1.id=t7.product_id','left')
...
//         $this->db->where(['t11.class'=>$class_id,'t1.is_deleted' => 'NOT_DELETED','t1.active'=>'1','t3.shop_id' =>$shop_id,'t13.group_id'=>$group_id]);
...
function fetch_data($group_id, $class_id, $category_pro_id, $pro_id, $id, $cids, $shop_id, $brand, $prop_filter, $sort_by, $limit, $start, $is_count, $isAll)
{
    $this->db
        ...
        ->join('products_photo t4', 't4.item_id=t1.id AND t4.active="1" AND t4.is_cover="1"', 'left')
        ->join('shops_coupons_offers t6', 't1.id=t6.product_id', 'left')
        ->join('class_pro_maps t11', 't1.id=t11.pro_id', 'left')
        ->join('cat_pro_maps t12', 't12.pro_id=t1.id', 'left')
        ->join('products_category t13', 't13.id=t12.cat_id', 'left');

    if (!empty($prop_filter)) {
        // Additional join logic if needed
    }
...
function fetch_data_search($group_id, $class_id, $search_val, $shop_id, $sort_by, $limit, $start, $is_count)
{
    $this->db
        ->select('t1.*, t1.name_portal as prod_name, t4.img, t4.thumbnail, t4.id as cover_id, t3.id as inventory_id, t3.mrp, t3.selling_rate, t3.qty as product_qty, t6.offer_upto, t6.discount_type, t7.is_featured, t8.product_id as wishlist_pid, t1.id as propid, t1.flag as ProductFlag')
        ->from('products_subcategory t1')
        ->join('shops_inventory t3', 't1.id=t3.product_id AND t3.is_deleted="NOT_DELETED" AND t3.status="1"', 'left')
        ->join('products_photo t4', 't4.item_id=t1.id AND t4.active="1" AND t4.is_cover="1"', 'left')
        ->join('shops_coupons_offers t6', 't1.id=t6.product_id', 'left')
...
//         ->select('t1.*, t1.name_portal as prod_name, t4.img,t4.thumbnail, t4.id as cover_id, t3.id as inventory_id, t3.mrp, t3.selling_rate, t3.qty as product_qty, t6.offer_upto, t6.discount_type, t7.is_featured, t8.product_id as wislist_pid,t1.id as propid,t1.flag as ProductFlag')
...
//         ->where(['t11.class'=>$class_id,'t1.is_deleted' => 'NOT_DELETED','t1.active'=>'1','t3.shop_id' =>$shop_id,'t13.group_id'=>$group_id]);
...
//     }
...

    {
        ...
        ->select('t1.*, t1.name_portal as prod_name, t4.img,t4.id as cover_id, t3.id as inventory_id, t3.mrp, t3.selling_rate, t5.name as cat_name,t3.qty as product_qty, t6.offer_upto, t6.discount_type,t7.is_featured, t8.product_id as wislist_pid')
...

    {
        ...
        ->select('t1.*, t1.id as product_id, t1.name_portal as prod_name, t4.img,t4.id as cover_id, t4.thumbnail, t3.mrp,t3.selling_rate, t3.id as inventory_id, t3.qty as product_qty, t6.offer_upto, t6.discount_type, t7.is_featured, t8.product_id as wislist_pid,t1.flag as ProductFlag')
       ...
       // ->join('flavour_master t9', 't9.id = t1.flavour_id AND t9.is_deleted="NOT_DELETED" AND t9.active="1"','left')
        ...
        ->where(['t1.is_deleted' => 'NOT_DELETED','t1.active'=>'1','t3.shop_id' =>$shop_id,'t3.selling_rate'])
