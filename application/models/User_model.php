...
class User_model extends CI_Model
{
    ...
    public function orders($uid,$shop_id)
    {
        ...
        // ->get();
		// return = $query->result();
        return $this->db->select("t1.delivery_charges,t1.id as oid,t1.house_no,t1.address_line_2,t1.address_line_3,t1.pincode,t1.city,t1.country,t1.email, t1.added, t1.total_value, t1.orderid, t1.status, t1.updated,t1.added as order_date, SUM(t2.qty) as item_qty, t3.name as status_name, t3.id as status_id")
            ->from('orders as t1')
            ->join('order_items t2', 't2.order_id = t1.id', 'left')
            ->join('order_status_master t3', 't3.id = t1.status','left')
  ...
  //       ->where(['t4.is_deleted' => 'NOT_DELETED','t1.id'=>$oid,'t5.is_cover' =>'1'])
        ...
        $order_items = $this->db->select('t2.qty as item_qty, t2.*, t4.id as product_id, t4.parent_cat_id, t1.is_parent, t4.name_portal as product_name, t4.unit_value, t4.unit_type, t5.img,t4.url as pro_url,t4.flag,t2.id as item_id')
            ->from('order_items t2')
...
//     }
    ...
    //     ->select('t2.*')
    //     ->from('order_items t1') 
    //     ->join('shops_inventory t2', 't2.product_id =t1.product_id','left')
